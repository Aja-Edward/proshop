...
export default function FlutterWave({ amount, name, email, orderId, paymentResult, onSuccess }) {
    ...
    return (
        <div className='fluttercontainer'>
            ...
            <Button
                ...
                onClick={() => {
                    handleFlutterPayment({
                        callback: (response) => {
                            onSuccess = dispatch(payOrder(orderId, paymentResult))
            ...
            <div> <em>Powered by</em> <span>FlutterWave</span></div>
        </div >
    );
}